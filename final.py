...
from PyQt5.QtWidgets import QApplication, QMainWindow,  QAction, QTextEdit, QFontDialog, QColorDialog, QFileDialog, QPushButton
import sys
...
class Ui_MainWindow(QtGui.QMainWindow):
    def setupUi(self, MainWindow):
        ...
        ############################################################################################
        self.pdf_export = QtWidgets.QPushButton(self.centralwidget)
        font = QtGui.QFont()
        font.setFamily("Yu Gothic UI")
        font.setPointSize(11)
        font.setBold(True)
        font.setWeight(75)
        self.pdf_export.setFont(font)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(
            "icons8-play-button-circled-96.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pdf_export.setIcon(icon)
        self.pdf_export.setIconSize(QtCore.QSize(50, 50))
        self.pdf_export.setObjectName("pdf_export")
        self.horizontalLayout.addWidget(self.pdf_export)
        self.pdf_export.clicked.connect(self.printpdf)
        ################################################################################################
        ...
        self.graphicsView.plotItem.setLimits(
            xMin=0, xMax=15, yMin=-0.7, yMax=0.7)
    ...
    def printpdf(self):
        ...
        if fn != '':
            ...
            self.MainWindow.print_(printer)
    ...
    def read_file1(self):
        self.fname1 = QtGui.QFileDialog.getOpenFileName(
            self, 'Open only txt or CSV or xls', os.getenv('HOME'), "csv(*.csv);; text(*.txt) ;; xls(*.xls)")
        path = self.fname1[0]
    ...
    def read_file2(self):
        fname = QtGui.QFileDialog.getOpenFileName(self, 'Open only txt or CSV or xls', os.getenv(
            'HOME'), "csv(*.csv);; text(*.txt) ;; xls(*.xls)")
        path = fname[0]
    ...
    def load1(self):
        ...
        self.data_line1 = self.graphicsView.plot(
            self.x1, self.y1, pen=self.pen)
        self.graphicsView.plotItem.setLimits(
            xMin=0, xMax=12, yMin=-0.6, yMax=0.6)
    ...
    def load2(self):
        ...
        self.data_line2 = self.graphicsView_2.plot(
            self.x2, self.y2, pen=self.pen)
        self.graphicsView_2.plotItem.setLimits(
            xMin=0, xMax=12, yMin=-0.6, yMax=0.6)
    ...
    def load3(self):
        ...
        self.data_line3 = self.graphicsView_3.plot(
            self.x3, self.y3, pen=self.pen)
        self.graphicsView_3.plotItem.setLimits(
            xMin=0, xMax=12, yMin=-0.6, yMax=0.6)
    ...
    def load4(self):
        ...
        self.data_line4 = self.graphicsView_4.plot(
            self.x4, self.y4, pen=self.pen)
        self.graphicsView_4.plotItem.setLimits(
            xMin=0, xMax=12, yMin=-0.6, yMax=0.6)
    ...
    def load5(self):
        ...
        self.data_line5 = self.graphicsView_5.plot(
            self.x5, self.y5, pen=self.pen)
        self.graphicsView_5.plotItem.setLimits(
            xMin=0, xMax=12, yMin=-0.6, yMax=0.6)
    ...
    def update_plot_data1(self):
        ...
        if self.x1[self.idx1] > 0.5:
            self.graphicsView.setLimits(xMin=min(x, default=0), xMax=max(
                x, default=0))  # disable paning over xlimits
    ...
    def update_plot_data2(self):
        ...
        if self.x2[self.idx2] > 0.5:
            self.graphicsView_2.setLimits(xMin=min(x, default=0), xMax=max(
                x, default=0))  # disable paning over xlimits
    ...
    def update_plot_data5(self):
        ...
        if self.x5[self.idx5] > 0.5:
            self.graphicsView_5.setLimits(xMin=min(x, default=0), xMax=max(
                x, default=0))  # disable paning over xlimits
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate(
            "MainWindow", "                                                                                   To zoom drag the graph to the right"))
